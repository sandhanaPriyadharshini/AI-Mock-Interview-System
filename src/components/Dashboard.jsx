...
function Dashboard() {
  ...
  if (savedResults.length > 0) {
    setLatest(savedResults[savedResults.length - 1]);
  } else {
    setLatest(null);
  }
}, []);
      ...
      <div className="flex-1 p-10">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="bg-zinc-900 p-8 rounded-3xl border border-zinc-800">
            ...
            <p className="text-5xl font-bold">{latest?.total || 0}</p>
          </div>

          <div className="bg-zinc-900 p-8 rounded-3xl border border-zinc-800">
            <h2 className="text-cyan-400 text-xl mb-4">Latest Score</h2>
            <p className="text-5xl font-bold">
              {latest?.score !== undefined ? `${latest.score}%` : "0%"}
            </p>
          </div>

          <div className="bg-zinc-900 p-8 rounded-3xl border border-zinc-800">
            <h2 className="text-cyan-400 text-xl mb-4">Latest Role</h2>
            <p className="text-3xl font-bold">{latest?.role || "N/A"}</p>
          </div>
        </div>
      </div>
    </div>
